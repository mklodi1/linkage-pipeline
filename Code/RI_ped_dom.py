...
> generates pedigree files for Reading Impairment patients under dominant inheritance model
...
with open("/lab01/Projects/Mudassir_Projects/linkage/Data/cohort_summary_edited_2023-06-27.txt", "r") as f1:
	...
	for line1 in f1:
            ...
            if len(PED_columns) > 1 and 'Father' not in line1:
                ...
                cohort_subjects.append(subject)

                if RI_phenotype == '2' or ASD_phenotype == '2':
                   phenotype = '2'
                elif RI_phenotype == 'x' or ASD_phenotype == 'x':
                   phenotype = '0'
                else:
                   phenotype = '1'
                ...
                filePath = '/lab01/Projects/Mudassir_Projects/linkage/Pedigree/dom/RI_ASD/fam_%s_RI_ASD_dom.ped' % (family)
                f2 = open(filePath, 'w')
                
                if PED_father == '0' and PED_mother == '0':
                        ancestors[family].append(PED_subject)
                        subpeds[family][PED_subject] = [PED_subject]

	# second gen iteration
	for family in family_data:
		individuals = family_data[family].values()
		subjects = family_data[family].keys()
		for subject in subjects:
			ind_data = family_data[family][subject]
			PED_father = ind_data[1]
			PED_mother = ind_data[2]
			if PED_father in subpeds[family].keys(): 
                        	subpeds[family][PED_father].append(subject)
			if PED_mother in subpeds[family].keys():
                        	subpeds[family][PED_mother].append(subject)
	...
	for family in family_data:
                ...
                for subject in subjects:
                        ind_data = family_data[family][subject]
                        PED_father = ind_data[0]
                        PED_mother = ind_data[1]
                        for perm in subped_perms:
                                if PED_father in perm:
                                        if subject not in perm:
                                                perm.append(subject)
                                        if PED_father not in perm:
                                                perm.append(PED_father)
                                        if PED_mother not in perm:
                                                perm.append(PED_mother)
                                if PED_mother in perm:
                                        if subject not in perm:
                                                perm.append(subject)
                                        if PED_father not in perm:
                                                perm.append(PED_father)
        ...
        # check to make sure no individuals were missed (handles all step-parents, children, etc)
	for family in family_data:
		individuals = family_data[family].values()
		subjects = family_data[family].keys()
		subped_perms = subpeds[family].values()
	...
	for family in subpeds:
		subped_perms = list(subpeds[family].values())
		keys = list(subpeds[family].keys())
	#print("Subpeds:",subpeds)
	...
	for family in subpeds:
            ...
            top = max(scores.items(), key=lambda x: x[1], default=0)
            if top != 0 :
                top_score = top[1]
                   ...
                   if subject not in family_data[family].keys():
                      ...
                      if subject in parents:
                        ...
                        if family_data[family][subject][1] in snv_sub.keys() and mother_gvf != '' and snv_sub[mother_gvf] != '':
                                ...
                                mother_gvf = family_data[family][subject][1]
                '''
                if subject not in cohort_subjects:
                        continue
                else:

...
dom_var_res = results_path + "/SNP_Analysis/Results/RI_ASD_dom_var.txt"
rec_var_res = results_path + "/SNP_Analysis/Results/RI_ASD_dom_var.txt"
dom_genes = results_path + "/SNP_Analysis/Results/RI_dom_genes_fam.txt"
rec_genes = results_path + "/SNP_Analysis/Results/RI_rec_genes_fam.txt"
SNP_genes = results_path + "/SNP_Analysis/Results/RI_SNP_genes.txt"
...
SV_non_benign = results_path + "/SV_Analysis/Results/RI_SV_inh_non_benign.txt"
SV_nb_genes = results_path + "/SV_Analysis/Results/RI_SV_genes_filt.txt"

gene_table = results_path + "/Gene_Analysis/RI_gene_table"
...
with open (SNP_genes) as f:
	for line in f:
		...
		RI_all_genes.append(gene)
		RI_SNV_genes.append(gene)

with open (CNV_genes) as f:
	for line in f:
		cols = line.split('\t')
		gene = cols[0].strip()
		count = str(cols[1].strip())
		#transcript = line[1]
		#HGNC_gene = transcript_gene[transcript]
		RI_all_genes.append(gene)
		RI_CNV_genes.append(gene)
...
with open (gene_table, 'w') as f6:
	...
	for gene_name in RI_all_genes:
		...
		if gene_name in known_NDD:
			...
			f6.write('0' + '\t')
		'''
		if gene_name in RI_SNV_genes and gene_name in pvalue.keys():
			if pvalue[gene_name] < 0.000485:
				f6.write('1' + '\t')
			else:
				f6.write('0' + '\t')
		else:
			f6.write('0' + '\t')
		'''
		if gene_name in RI_SNV_genes:
			f6.write('1' + '\t')
		else:
			f6.write('0' + '\t')

		if gene_name in RI_CNV_genes:
			f6.write('1' + '\t')
		else:
			f6.write('0' + '\t')

		if gene_name in RI_SV_genes:
			f6.write('1' + '\t')
		else:
			f6.write('0' + '\t')
